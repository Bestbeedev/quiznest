...
async function effectiveLimit(organizationId: string, baseLimit: number | null, effect: "EXTRA_QUIZZES" | "EXTRA_PARTICIPANTS" | "EXTRA_AI_GENERATIONS") {
...
export function getEffectiveQuestionLimit(organizationId: string, baseLimit: number | null) {
  return effectiveLimit(organizationId, baseLimit, "EXTRA_AI_GENERATIONS");
}
